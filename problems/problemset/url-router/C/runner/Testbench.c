...
int main(void){
    ...
    while(tok){
        if(strcmp(tok,"REGISTER")==0){
            ...
            router_register(r,method,pattern);
        } else if(strcmp(tok,"MATCH")==0){
            char *method=strtok(NULL," \t\n\r");
            char *url=strtok(NULL," \t\n\r");
            RouteMatch m=router_match(r,method,url);
            if(m.pattern[0]=='\0'){
                b_cat(&out,"NOTFOUND\n");
            } else {
                b_cat(&out,method);b_cat(&out," ");b_cat(&out,m.pattern);
                for(int i=0;i<m.param_count;i++){
                    b_cat(&out," ");b_cat(&out,m.param_keys[i]);
                    b_cat(&out,"=");b_cat(&out,m.param_vals[i]);
                }
                b_cat(&out,"\n");
            }
        }
