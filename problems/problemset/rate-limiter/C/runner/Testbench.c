...
int main(void) {
    ...
    while(tok){
        if(strcmp(tok,"INIT")==0){
            int maxr=atoi(strtok(NULL," \t\n\r"));
            long long win=atoll(strtok(NULL," \t\n\r"));
            if(rl) rl_destroy(rl);
            rl=rl_create(maxr,win);
        } else if(strcmp(tok,"REQUEST")==0){
            long long ts=atoll(strtok(NULL," \t\n\r"));
            b_cat(&out, rl_allow(rl,ts) ? "ALLOWED\n" : "DENIED\n");
        }
        tok=strtok(NULL," \t\n\r");
    }
